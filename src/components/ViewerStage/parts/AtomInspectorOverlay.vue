<template>
    <!-- Mini collapsed bar -->
    <div v-if="visible && collapsed" class="atom-inspector-mini" role="button" tabindex="0" @click="collapsed = false"
        @keydown.enter.prevent="collapsed = false" @keydown.space.prevent="collapsed = false">
        <a-button class="mini-handle" size="small">⟨</a-button>
    </div>

    <!-- Drawer -->
    <a-drawer v-if="visible" :placement="placement" :open="visible && !collapsed" :closable="false" :mask="false"
        :get-container="false" :style="drawerRootStyle" :contentWrapperStyle="contentWrapperStyle"
        :bodyStyle="drawerBodyStyle" :width="placement === 'right' ? desktopWidth : undefined"
        :height="placement === 'bottom' ? mobileHeight : undefined">
        <!-- Resize handle -->
        <div class="atom-inspector__resizer" :class="placement === 'right' ? 'is-right' : 'is-bottom'"
            ...
            <div v-else class="atom-inspector__content">
                ...
                <div class="atom-inspector__footer">
                    ...
                    <a-descriptions size="small" :column="1">
                        <a-descriptions-item v-if="measureMode && measure.distance12 != null"
                            :label="`${t('viewer.inspect.distance')} (1–2)`">
                            {{ fmt(measure.distance12) }} Å
                        </a-descriptions-item>

                        <a-descriptions-item v-if="measureMode && measure.distance23 != null"
                            :label="`${t('viewer.inspect.distance')} (2–3)`">
                            {{ fmt(measure.distance23) }} Å
                        </a-descriptions-item>

                        <a-descriptions-item v-if="measureMode && measure.angleDeg != null"
                            :label="t('viewer.inspect.angle')">
                            {{ fmt(measure.angleDeg) }}°
                        </a-descriptions-item>
                    </a-descriptions>

                    <a-typography-text v-if="measureMode && selected.length > 1" type="secondary"
